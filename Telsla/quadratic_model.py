...
class DRagent_Quad(nn.Module):
    ...
    def __init__(self, P1, P2, T, type='matrix'):
        ...
        if type=='matrix':
            ...
            obj = (lambda d, p, price, c1, c2, c3, c4, E1, E2, eta1, eta2, e0: -price @ (d - p) + torch.ones(T)@ (c1*d) + torch.ones(T) @ (c3*p) + cp.sum_squares(c2*d) + cp.sum_squares(c4*p)
             if isinstance(d, cp.Variable)
            else -price @ (d - p) +  torch.ones(T)@ (c1*d) + torch.ones(T) @ (c3*p) + torch.sum((c2*d)**2) + torch.sum((c4*p)**2))
...
def train(dataset, T=24, N_train=20, N_test=10, model_type='matrix'):
    ...
    for ite in range(150):
        ...
        print(f'ite = {ite}, loss = {loss.detach().numpy()}, val_l = {loss2.detach().numpy()}, eta1 = {layer.eta1.detach().numpy()}, eta2 = {layer.eta2.detach().numpy()},')
...
if __name__ == '__main__':
    ...
    torch.save(layer.state_dict(), f'result/model_gradient/quad/model_{model_type}1.pth')
    np.savez(f'result/model_gradient/quad/loss_{model_type}1.npz', loss=L, val_loss=val_L)