...
class DRagent_Quad(nn.Module):
    ...
    def __init__(self, P1, P2, T, type='matrix'):
        ...
        self.ineq12 = (lambda d, p, price, c1, c2,  c3, c4,  E1, E2, eta1, eta2, e0: p - torch.ones(T//2, dtype=torch.double) * P1)
        self.ineq22 = (lambda d, p, price, c1, c2,  c3, c4,  E1, E2, eta1, eta2, e0: torch.ones(T//2, dtype=torch.double)* 1e-9 - p)
        self.ineq32 = (lambda d, p, price, c1, c2,  c3, c4,  E1, E2, eta1, eta2, e0: d - torch.ones(T//2, dtype=torch.double) * P2)
        self.ineq42 = (lambda d, p, price, c1, c2,  c3, c4,  E1, E2, eta1, eta2, e0: torch.ones(T//2, dtype=torch.double) * 1e-9 - d)
        self.ineq52 = (lambda d, p, price, c1, c2,  c3, c4,  E1, E2, eta1, eta2, e0: torch.tril(torch.ones(T//2, T//2, dtype=torch.double)) @ (eta1 * p / 4.44 - d / 4.44 / eta2)
            - torch.as_tensor(np.arange(eps, (T//2+1)*eps, eps))
            - torch.ones(T//2, dtype=torch.double) * (E1-e0))
        self.ineq62 = lambda d, p, price, c1, c2, c3, c4,   E1, E2, eta1, eta2, e0: torch.ones(
            T//2, dtype=torch.double
        ) * (E2-e0) - torch.tril(torch.ones(T//2, T//2, dtype=torch.double)) @ (eta1 * p / 4.4- d /4.4 / eta2) + torch.as_tensor(np.arange(eps, (T//2+1)*eps, eps))
        ...
        self.layer2 = util.OptLayer(
            [cp.Variable(T//2), cp.Variable(T//2)],
            [cp.Parameter(T//2),  cp.Parameter(),  cp.Parameter(), cp.Parameter(),  cp.Parameter(), cp.Parameter(1), cp.Parameter(1), cp.Parameter(), cp.Parameter(), cp.Parameter(1),],
            obj,
            [self.ineq12, self.ineq22, self.ineq32, self.ineq42, self.ineq52, self.ineq62],
            [],
            solver="GUROBI",
            verbose=False,
        )

    def forward(self, price, e0, e1):
        dr_p, dr_d = self.layer(
            price,
        ...
        price2 = price[:,self.T//2:]
        dr_p2, dr_d2 = self.layer2(
            price2,
            self.c1.expand(price2.shape[0], *self.c1.shape),
            self.c2sqrt.expand(price2.shape[0], *self.c2sqrt.shape),
            self.c3.expand(price2.shape[0], *self.c3.shape),
            self.c4sqrt.expand(price2.shape[0], *self.c4sqrt.shape),
            self.E1.expand(price2.shape[0], *self.E1.shape),
            self.E2.expand(price2.shape[0], *self.E2.shape),
            self.eta1.expand(price2.shape[0], *self.eta1.shape),
            self.eta2.expand(price2.shape[0], *self.eta2.shape),
            e1
        ) 
        dr_p[:,self.T//2:] = dr_p2
        dr_d[:,self.T//2:] = dr_d2
...
def train(dataset, T=24, N_train=20, N_test=10, model_type='matrix'):
    ...
    e1_tensor = torch.from_numpy(e1[N_test:N_test+N_train]).double()
    ...
    e1_tensor2 = torch.from_numpy(e1[:N_test]).double()
    ...
    layer.load_state_dict(torch.load('result/model_gradient/quad/model_scalar2.pth'))
    opt1 = optim.Adam(layer.parameters(), lr=1e-1)
    #figure, axes = plt.subplots(1,2, figsize=(8,4))
    for ite in range(500):
        dp_pred = layer(price_tensor, e0_tensor, e1_tensor)
        if ite == 50:
        ...
        layer.eval()
        dp_pred2 = layer(price_tensor2, e0_tensor2, e1_tensor2)
...
if __name__ == '__main__':
    ...
    data = np.load(f"dataset/UQ-Tesla-2020_2.npz")
    L, val_L, layer = train(data, T=80, N_train=21, N_test=20, model_type=model_type)
    torch.save(layer.state_dict(), f'result/model_gradient/quad/model_{model_type}2.pth')
    np.savez(f'result/model_gradient/quad/loss_{model_type}2.npz', loss=L, val_loss=val_L)