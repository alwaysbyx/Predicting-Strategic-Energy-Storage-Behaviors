...
class DRagent(nn.Module):
    ...
    def __init__(self, P1, P2, T, type='scalar'):
        ...
        self.E1 = 1.*torch.ones(1) # 1.*torch.ones(1) # nn.Parameter(0.5 * torch.ones(1))
        self.E2 =  0.*torch.ones(1)  # 0.*torch.ones(1) # nn.Parameter(-0.5 * torch.ones(1))
...
def train(dataset, i, T=24,  N_train=20, N_test=10,  model_type='vector'):
    ...
    for ite in range(200):
        ...
        if ite % 50 == 0:
            torch.save(layer.state_dict(), f'result/model_gradient/ICNN/model_{model_type}_E.pth')
...
if __name__ == '__main__':
    ...
    torch.save(layer.state_dict(), f'result/model_gradient/ICNN/model_{model_type}_E.pth')
    np.savez(f'result/model_gradient/ICNN/loss_{model_type}_E.npz', loss=L, val_loss=val_L)