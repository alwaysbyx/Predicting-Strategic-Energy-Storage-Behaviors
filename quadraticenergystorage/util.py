...
class DRagent_Quad(nn.Module):
    ...
    def __init__(self, P1, P2, T):
        ...
        obj = (lambda d, p, price, c1, c2, E1, E2, eta: -price @ (d - p) + c1 * cp.sum(d)  + cp.sum_squares(cp.sqrt(c2) * d) if isinstance(d, cp.Variable) else -price @ (d - p) + c1 * torch.sum(d) + c2 * torch.sum(d**2))
        ...
        parameters = [cp.Parameter(T,), cp.Parameter(1), cp.Parameter(1), cp.Parameter(1), cp.Parameter(1), cp.Parameter(1),]
        self.layer = OptLayer(
